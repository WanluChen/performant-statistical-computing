...
v <- rnorm(ar_length)
rcpp.output <- ar_precision_matvec(v, auto_corr)
r.output <- ar_precision_matvec_R(v, auto_corr) 
assert("The output of Rcpp function `ar_precision_matvec` is the same as R function:", {
  are_all_close(rcpp.output, r.output)
})
```
...
r.output <- A %*% B
rcpp.output <- row_col_dot_matmat(A, B)
assert("The output of Rcpp function `row_col_dot_matmat` is the same as R function:", {
  are_all_close(r.output, rcpp.output)
})
```
...
# Check the output of `matmat_via_eigen`
r.output <- A %*% B
rcpp.output <- matmat_via_eigen(A, B)
assert("The output of Rcpp function `matmat_via_eigen` is the same as R function:", {
  are_all_close(r.output, rcpp.output)
})
