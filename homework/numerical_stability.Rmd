...
gauss_conditional_mean <- function(y_2, mu_1, mu_2, cov_12, cov_22) {
  ...
  # get k
  var_cum = cumsum(eigen_val)/sum(eigen_val)
  k = min(which(var_cum > 1 - 1e-5))
  # compute sigma^-1
  sigma_inv = Reduce('+', lapply(1:k, function(i) {
    (eigen_val[i]^-1*(eigen_vec[, i] %*% t(eigen_vec[, i])))
  }))
  
  return(mu_1 + cov_12 %*% sigma_inv %*% (y_2 - mu_2))
}
...
# predict
y_pred <- gauss_conditional_mean(y_obs, mean_new, mean_obs, Sigma_cross, Sigma_obs)
```
...
eigen_val = eigen_decomp$values
var_cum = cumsum(eigen_val)/sum(eigen_val)
k = min(which(var_cum > .9999))
sigma_inv = Reduce('+', lapply(1:k, function(i) {
  (eigen_val[i]^-1*(eigen_vec[, i] %*% t(eigen_vec[, i])))
}))
var_pred <- Sigma_new - Sigma_cross %*% sigma_inv %*% t(Sigma_cross)
...
y_pred <- mean_new + Sigma_cross %*% (sigma_inv %*% (y_obs - mean_obs))
var_pred <- Sigma_new - Sigma_cross %*% sigma_inv %*% t(Sigma_cross)
ci_lower <- y_pred - 1.96*sqrt(diag(var_pred))
ci_upper <- y_pred + 1.96*sqrt(diag(var_pred))
...
```{r, fig.dim=c(8, 5), fig.align='center', results='hold'}
# Plot
plot(
  loc_obs, y_obs, 
  xlab="s", ylab="y(s)", 
...
points(loc_new, y_pred_solve, col = jhu_color$orange)
lines(loc_new, ci_lower_solve, col = jhu_color$orange)
lines(loc_new, ci_upper_solve, col = jhu_color$orange)
```
