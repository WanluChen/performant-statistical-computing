...
ar_precision_matvec <- function(v, auto_corr) {
  ...
  v1 = v[1] - auto_corr * v[2]
  v_ = c(-auto_corr, 1+auto_corr^2, -auto_corr) %*% rbind(v[1:(n-2)], v[2:(n-1)], v[3:n])
  vn = - auto_corr * v[n-1] + v[n]
  
  v = c(v1, v_, vn) / (1 - auto_corr^2)
...
eigen_val = eigen(Sigma)$values[1:10]
eigen_vec = eigen(Sigma)$vectors[1:10]
...
# Compare
assert("The two results are similar.", {
                are_all_close(lancos_eigen_val, eigen_val)
                are_all_close(abs(lancos_eigen_vec), abs(eigen_vec), rel_tol = 5e-2)
})
```
