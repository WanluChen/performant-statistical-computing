...
NumericVector ar_precision_matvec(const NumericVector& v, double auto_corr) {
	...
	result[0] = (v[0] - auto_corr * v[1]) / (1 - auto_corr_sq);
	result[n-1] = (-auto_corr * v[n-2] + v[n-1]) / (1 - auto_corr_sq);
	for(int i = 1; i < n-1; ++i){
		result[i] = (-auto_corr * (v[i-1] + v[i+1]) + (1 + auto_corr_sq) * v[i]) / (1 - auto_corr_sq);
	}
