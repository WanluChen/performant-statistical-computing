...
At first the error decreases because when $\Delta x$ gets smaller, the approxixmation gets more accurate. Then the error increases again because the precision can not be guaranteed.

**Bonus question 1:** What is the slope in the "latter part" of your plotted line?

The slope in the "latter part" of your plotted line: 1.
...
Oops, that didn't work --- figure out what went wrong. - *It returns "function cannot be evaluated at initial parameters"*.

After you figured it out, don't worry about fixing it but try starting the optimization from $\beta = (0, 0, ..., 0)$. It should at least run now. - *Yes*.

Check `optim_result$convergence` and see if we were successful in finding the penalized MLE this time. - *Yes*.
...
result.0 <- tryCatch(
  {
    ...
    cat("`optim_result$convergence`:", optim_result$convergence, "\n")
  },
  error=function(cond) {
    message(paste("Warning: The algorithm did not converged."))
    return(NA)
  }
)
```
...
approximate_grad <- function(func, x, dx = 10^-6) {
  ...
  for (i in 1:length(x)) {
    numerical_grad[i] <- (func(x+dx) - func(x-dx)) / (2*dx)
  }
...
Is the optimization successful this time? - *Yes.*
...
approx_directional_deriv <- function(func, x, v) {
  e = 10^-6
  deriv = (func(x+e*v)-func(x))/e
  return(deriv)
}
...
assertion <- ""
assert(assertion, {
  approx_deriv <- approx_directional_deriv(compute_internet_ad_ploglik, reg_coef, v)
  deriv <- compute_internet_ad_grad(reg_coef)
  are_all_close(deriv, approx_deriv)
})
